...
namespace test_unexpected {
    ...
    constexpr void test() {
        ...
        using Unexpect = zeus::unexpected<test_error>;
        ...
        const auto converted = zeus::unexpected<convertible>{convertible{3}};
        ...
        // deduction guide
        zeus::unexpected deduced(test_error{42});
        static_assert(is_same_v<decltype(deduced), Unexpect>);
    }
...
namespace test_expected {
    constexpr void test_aliases() {
...

        {
            ...
            static_assert(is_same_v<typename Expected::value_type, value_tag>);
            static_assert(is_same_v<typename Expected::error_type, error_tag>);
            static_assert(is_same_v<typename Expected::unexpected_type, zeus::unexpected<error_tag>>);

            static_assert(is_same_v<typename Expected::rebind<int>, expected<int, error_tag>>);
        }

        {
            using Expected = expected<void, error_tag>;
            static_assert(is_same_v<typename Expected::value_type, void>);
            static_assert(is_same_v<typename Expected::error_type, error_tag>);
            static_assert(is_same_v<typename Expected::unexpected_type, zeus::unexpected<error_tag>>);
    ...
    constexpr void test_constructors() noexcept {
        ...
        { // converting from unexpected
            using Input    = zeus::unexpected<convertible>;
    ...
    constexpr void test_assignment() noexcept {
        ...
        { // assign error type const ref
            constexpr bool should_be_noexcept = nothrow_copy_constructible && nothrow_copy_assignable;
            using Expected                    = expected<int, payload_assign>;
            using Unexpected                  = zeus::unexpected<payload_assign>;
        ...
        { // assign convertible error const ref
            constexpr bool should_be_noexcept = nothrow_copy_constructible && nothrow_copy_assignable;
            using Expected                    = expected<int, payload_assign>;
            using Unexpected                  = zeus::unexpected<convertible>;
            const Unexpected input_error{42};
    ...
    constexpr void test_equality() noexcept {
        ...
        { // compare against unexpected with same base
            using Base       = payload_equality;
            using Unexpected = zeus::unexpected<Base>;
        ...
        { // compare against unexpected with different base
            using Base       = payload_equality;
            using Unexpected = zeus::unexpected<int>;
            using Expected   = expected<int, Base>;
